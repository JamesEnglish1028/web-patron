...
const pickAuthor = (metadata: any): string => {
  ...
  if (Array.isArray(metadata?.contributor)) {
    const primary =
      metadata.contributor.find((entry: any) => {
...
const extractCoverImageUrl = (
  ...
  for (const candidate of candidates) {
    const rels = Array.isArray(candidate?.rel)
      ? candidate.rel
      : [candidate?.rel];
    if (
      rels.some((value: any) =>
...
export const parseAudiobookManifest = (
  ...
  const tracks: AudiobookTrack[] = readingOrder
    .map((entry: any, index: number) => {
      const href =
        typeof entry === "string" ? entry : String(entry?.href || "").trim();
      if (!href) return null;
      const title =
    ...
    .filter(
      (entry: AudiobookTrack | null): entry is AudiobookTrack => entry !== null
    );
  ...
  const toc: AudiobookTocItem[] = (
    Array.isArray(payload?.toc) ? payload.toc : []
  )
    .map((entry: any) => {
    ...
    .filter(
      (entry: AudiobookTocItem | null): entry is AudiobookTocItem =>
        entry !== null
    );
  ...
  return {
    title:
      String(
        metadata?.title || payload?.title || "Untitled Audiobook"
